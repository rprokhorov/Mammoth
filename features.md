# Mattermost Desktop Client — Implementation History

## Phases Completed

| Phase | Name | Status |
|-------|------|--------|
| 0 | Scaffolding & Infrastructure | Done |
| 1 | Authentication & Server Connection | Done |
| 2 | Teams, Channels & WebSocket | Done |
| 3 | Messaging (Core) | Done |
| 4 | Threads (CRT) | Done |
| 5 | Notifications & Presence | Done |
| 6 | Files & Search | Done |
| 7 | Profile & Settings | Done |
| 8 | Emoji, Reactions & Message Management | Partial |
| 9 | Channel Organization | Partial |
| 10 | System Tray, Multi-Window & Deep Linking | Partial |
| 11 | Calls Integration | Not started |
| 12 | Integrations, Polish & Performance | Partial |

---

## Chronological Log

### Phase 0 — Scaffolding (2026-03-31)
- Tauri 2 project scaffolded (React + TS + Vite + Rust)
- Directory structure, ESLint, Prettier, clippy, Vitest, path aliases
- GitHub Actions CI/build workflows
- Mattermost test server deployed via Docker (localhost:8065)
- **No issues**

### Phase 1 — Authentication (2026-03-31)
- `MattermostClient` (reqwest): ping, login, logout, get_me
- Server registry with persistent config, token storage
- Login UI: server URL input, username/password form, PAT option
- Multi-server shell with server icon rail
- **No issues**

### Phase 2 — Teams, Channels & WebSocket (2026-03-31)
- REST: teams, channels, view_channel
- WebSocket manager (tokio-tungstenite): reconnect, heartbeat, event parsing
- Channel list UI: grouped public/private/DM, unread indicators
- Team switching, Zustand + Rust AppState wiring
- **No issues**

### Phase 3 — Messaging (2026-03-31 to 2026-04-01)
- REST: get/create/update/delete posts
- Virtualized message list (@tanstack/react-virtual), date separators, infinite scroll
- Markdown rendering (react-markdown + remark-gfm), code blocks, @mentions
- Message composer: multi-line, @mention autocomplete, ~channel autocomplete
- Real-time WS events: posted/edited/deleted
- SQLite message cache
- **No issues during implementation, but bugs found during testing (see below)**

### Phase 4 — Threads (2026-04-01)
- REST: get_post_thread, get_user_threads, follow/unfollow, mark_thread_as_read
- Thread panel (right side): root post + replies + composer
- Reply button, reply count badge on messages
- Threads view in sidebar: all followed threads, unread highlight
- **No issues during implementation, bugs found later during testing**

### Phases 5-9 — Bulk implementation (2026-04-01)
- Phase 5: Desktop notifications, badge count, presence, typing indicators
- Phase 6: File upload (drag-and-drop, clipboard paste), inline image preview, lightbox, file download, search (Cmd+K)
- Phase 7: User popover, profile modal, status picker, settings page
- Phase 8: Emoji picker, reactions bar, pin/unpin, saved messages
- Phase 9: Favorites, channel create/archive, context menu
- **No issues during implementation**

### Phase 10 — System Tray (2026-04-01, partial)
- System tray: icon, menu (Show/Hide/Quit), close-to-tray
- Badge count via window.set_badge_count()
- **Workaround:** `image` crate failed to download from crates.io (network issue). Used `png` crate directly for tray icon decoding.
- **Deferred:** Deep linking (tauri-plugin-deep-link download failed), auto-update (tauri-plugin-updater download failed), multi-window pop-out

### Deferred Features Batch (2026-04-01)
Implemented in one pass:
- Emoji autocomplete in composer (`:query` pattern, dropdown, arrow keys)
- Favorites group in channel list
- Per-channel notification prefs (context menu: All/Mentions/Mute)
- Badge count (dock badge + tray tooltip)
- Custom status (emoji + text + expiry picker)
- Performance: React.memo on MessageItem, lazy-loaded modals, useMemo/useCallback
- Accessibility: focus-visible, aria-labels, semantic nav elements

### Security & Tooling Hardening (2026-07-07)
Аудит репозитория и закрытие найденных проблем одной серией:
- **XSS**: вывод `marked` шёл в `dangerouslySetInnerHTML` без санитизации (raw HTML из сообщений исполнялся в webview с доступом к Tauri IPC). Добавлен DOMPurify + строгий CSP в tauri.conf.json (`script-src 'self'`, img `data:`/`blob:`, inline-стили только для mermaid). 15 регрессионных тестов.
- **Токены**: перенесены из plaintext `servers.json` в OS keychain (crate `keyring`: macOS Keychain / Windows Credential Manager / Linux Secret Service) с автоматической миграцией при старте и фолбэком на конфиг.
- **CI**: шаг фронтенд-тестов заканчивался `|| true` — падения никогда не валили сборку; убрано, добавлен шаг ESLint. Rust-гейты (fmt/clippy `-D warnings`) были красными — применён `cargo fmt` ко всему бэкенду, закрыты 24 clippy-предупреждения.
- **Линт**: скрипты `lint`/`format` ссылались на несуществующие ESLint/Prettier — добавлены flat config + зависимости. Найдено реальное нарушение Rules of Hooks (два `useMemo` после раннего `return` в ThreadPanel).
- **Прочее**: препроцессинг `@mention`/`~channel`/`:emoji:` больше не портит код-блоки; версия Cargo.toml синхронизирована (0.2.0 → 0.4.0) и bump-скрипт теперь бампит её тоже; удалён случайно закоммиченный `localhost-recording.json` (490 КБ); заполнен пустой LICENSE; удалён неиспользуемый `react-router-dom`.

---

## Bugs & Fixes

### Resolved on first attempt

| Bug | Description | Root Cause | Fix |
|-----|-------------|------------|-----|
| Blank screen after login | No channels/messages/profile after connecting | `handleLoginSuccess` not called on session restore | Added teams/channels loading after session validation |
| Channel list empty | Channels loaded from API but not displayed | `#[serde(rename = "type")]` serialized as `"type"` instead of `"channel_type"`, frontend filter never matched | Changed to `#[serde(alias = "type")]` |
| Login form missing on startup | Infinite re-renders, blank screen | Zustand store destructuring in useEffect caused infinite loops | Replaced with `getState()` calls inside effects |
| Thread navigation broken | Clicking channel after Threads view doesn't switch | `handleSelectChannel` didn't clear `mainSubView` | Added `store.setMainSubView("channels")` in channel selection |
| Unread count persists | Badge not cleared when entering a channel | Badge only cleared after async server response | Added immediate `updateChannelMentions(channelId, 0, 0)` before server call |
| Thread read-marking broken | `mark_thread_as_read` invoke failing silently | Missing `teamId` parameter in ThreadPanel's invoke call | Read `activeTeamId` from `useUiStore` inside ThreadPanel |

### Required multiple attempts / user had to re-request

| Bug | Attempts | What happened |
|-----|----------|---------------|
| **Login form disappears (2nd occurrence)** | 2 | First reported after Phase 4 build. Initial fix (killed stale process on port 1420) didn't resolve it. Context window exhausted mid-debug. User re-reported after context resume. Root cause was deeper: Rules of Hooks violation + Zustand subscriptions to entire store object. Fixed by converting all `useUiStore()` to individual selectors `useUiStore((s) => s.field)`. |
| **Thread replies showing in main channel** | 2 | User reported but context window was exhausted immediately after. User had to re-send the exact same request in the next session. Fix: added `if (post.root_id) continue` in MessageList, changed WS handler to use `updatePost` instead of `addPost` for thread replies. |
| **Thread reply count not shown** | 2 | User sent request, no response appeared (slow processing). User re-sent 4 minutes later. Fix: computed reply counts from loaded posts in `setChannelPosts`, added `incrementReplyCount` in WS handler. |
| **Threads unread badge not appearing** | 2 | **Attempt 1:** User reported badge only appears after opening Threads view. Fix: added `loadUserThreads()` on app startup. **Attempt 2:** User reported it still doesn't work when in a channel/DM. Root cause: `incrementThreadUnread` silently returned when thread wasn't in the local `userThreads` array. Fix: changed to always increment `userThreadsUnread` even when thread not found locally. |

---

## Запросы пользователя

Исправления и фичи, запрошенные пользователем во время тестирования, переформулированные как описание функциональности.

| # | Запрос | Статус | Попыток |
|---|--------|--------|---------|
| 1 | После авторизации приложение должно автоматически загружать и отображать команды, каналы и профиль пользователя | Готово | 1 |
| 2 | Список каналов должен корректно отображать все каналы с сервера (публичные, приватные, ЛС) | Готово | 1 |
| 3 | Приложение должно восстанавливать сессию при запуске без повторного ввода логина и пароля | Готово | 1 |
| 4 | Ответы в тредах должны отображаться только в панели треда, а не в основном списке сообщений канала | Готово | 2 (потеря контекста) |
| 5 | Клик по каналу из вида «Треды» должен переключать на канал; кнопка «Треды» должна снимать выделение с активного канала | Готово | 1 |
| 6 | Сообщения в канале должны показывать индикатор количества ответов в треде | Готово | 2 (повторная отправка) |
| 7 | Бейдж непрочитанных на канале должен считаться только по корневым сообщениям; ответы в тредах увеличивают счётчик раздела «Треды» | Готово | 1 |
| 8 | Бейдж непрочитанных на канале должен сбрасываться сразу при входе в канал | Готово | 1 |
| 9 | Подписанные треды должны показывать количество непрочитанных ответов, а открытие треда должно отмечать его как прочитанный | Готово | 1 |
| 10 | Бейдж непрочитанных тредов должен обновляться в реальном времени независимо от текущего вида (канал, ЛС или «Треды») | Готово | 2 (первый фикс не покрыл треды, отсутствующие в локальном кеше) |
| 11 | В списке сообщений не должно быть горизонтальной прокрутки; в поле ввода не должно быть вертикальной прокрутки, максимум 6 строк | Готово | 1 |
| 12 | Всплывающее окно выбора статуса должно закрываться при клике в любое место вне него | Готово | 1 |
| 13 | Левый клик по каналу должен менять содержимое дефолтного таба, а новый таб создаётся только по среднему клику или через контекстное меню | Готово | 1 |
| 14 | Поддержка авторизации через GitLab SSO в форме логина (открытие SSO-окна, захват токена, автоматический вход) | Готово | 8 (eval() не работает на внешних URL в Tauri, reqwest не имеет сессионных куков webview, mobile_login endpoint не триггерит on_navigation для mmauth://. Решение: Tauri cookies_for_url() API для чтения HttpOnly куков напрямую из cookie store webview) |
| 15 | Сообщения в панели треда должны быть отсортированы по времени (от старых к новым) | Готово | 1 |
| 16 | В панели треда должны отображаться реакции на сообщения и должна работать возможность добавлять реакции | Готово | 1 |
| 17 | В сообщениях должны отображаться прикреплённые изображения с возможностью увеличить по клику | Готово | 1 |
| 18 | Сообщения должны поддерживать полную Markdown-разметку (заголовки, списки, таблицы, blockquote, код) | Готово | 1 |
| 19 | Блоки кода не должны вызывать горизонтальную прокрутку — длинные строки переносятся | Готово | 1 |
| 20 | Диалог 1 на 1 должен отображаться с именем собеседника, а не текущего пользователя | Готово | 1 |
| 21 | Каналы в боковой панели должны быть разбиты по категориям, созданным пользователем в Mattermost | Готово | 2 (API возвращает обёртку {categories:[...]}, а не массив) |
| 22 | Возможность прикреплять файлы и изображения к сообщениям (кнопка 📎, drag-and-drop, вставка из буфера); превью картинок высотой равной строке ввода | Готово | 2 (фильтр типов файлов блокировал выбор на macOS; стили превью отсутствовали) |
| 23 | Кнопка выбора смайлика 😊 в поле ввода сообщения и в панели треда; вставка эмодзи в позицию курсора | Готово | 2 (пикер закрывался сразу из-за конфликта mousedown; fix: triggerRef в EmojiPicker) |
| 24 | Клик средней кнопкой мыши по корневому сообщению открывает панель тредов; повторный клик по другому сообщению переключает тред | Готово | 1 |
| 25 | При открытии канала список сообщений должен оставаться прокрученным до последнего сообщения, даже когда картинки догружаются асинхронно | Готово | 3 (ResizeObserver не помог; placeholder-высота не помогла; fix: onLoad на img → scrollTop=scrollHeight если у дна) |
| 26 | Закрытие лайтбокса с картинкой по нажатию Esc | Готово | 1 |
| 27 | Изменение ширины панелей (каналы, чат, треды) перетаскиванием разделителя | Готово | 1 |
| 28 | Тёмные скроллбары под тёмную тему (color-scheme: dark + явные цвета трека) | Готово | 2 (transparent трек не работал в WebKit) |
| 29 | Быстрая загрузка картинок: thumbnail показывается сразу, оригинал подгружается фоново и заменяет превью; оба кэшируются в памяти | Готово | 2 (первый вариант загружал оригинал только при клике) |
| 30 | Поддержка кастомных эмодзи сервера: отображение в сообщениях как img, вкладка ★ в пикере, кэширование иконок | Готово | 1 |
| 31 | Исправить пагинацию кастомных эмодзи (per_page снижен до 100 — реальный лимит API); расширить стандартный набор эмодзи с ~80 до ~700+ по категориям | Готово | 2 (пустые :name: в гриде; недостающие People-маппинги) |
| 32 | Пикер эмодзи (в composer, треде и реакциях) должен открываться вверх или вниз в зависимости от места на экране, не уходя за его пределы | Готово | 2 (composer/thread исправлены отдельно; реакции в MessageItem исправлены следующим шагом) |
| 33 | Поле ввода сообщения растягивается до 20 строк без вертикального скролла (в composer и в панели тредов) | Готово | 1 |
| 34 | Кастомные эмодзи отображаются в реакциях на сообщения (вместо текста :name: рендерится img) | Готово | 1 |
| 35 | При сетевой ошибке во время запуска приложение не выкидывает на форму логина — токен сохраняется, приложение открывается в обычном режиме | Готово | 1 |
| 36 | При наведении на реакцию показывается имя пользователя (nickname / First Last / username), а не логин | Готово | 1 |
| 37 | Аватарки пользователей отображаются в сообщениях, иконке профиля в хедере и в настройках профиля; при отсутствии фото — инициалы | Готово | 1 |
| 38 | Аватарка собеседника отображается в списке приватных диалогов (DM) в боковой панели | Готово | 1 |
| 39 | Карточка пользователя: аватарка, "last seen", custom attributes из props (Markdown) | Готово | 1 |
| 40 | При недоступном сервере список сообщений показывает ошибку и кнопку "Повторить" вместо пустого экрана | Готово | 1 |
| 41 | Кастомные эмодзи кэшируются на 10 минут — повторные запуски не перезагружают 5000+ эмодзи | Готово | 1 |
| 42 | Карточка пользователя открывается мгновенно из локального кэша; position/roles/props догружаются в фоне | Готово | 1 |
| 43 | Custom Attributes плагин: данные рендерятся через CustomEmojiRenderer; фиксированная ширина карточки 280px, перенос строк | Готово | 1 |
| 44 | Ссылки в карточке (и везде в приложении) открываются в системном браузере через глобальный перехватчик + Rust open_url | Готово | 1 |
| 45 | Карточка пользователя: кнопка "💬 Message" открывает DM с пользователем; кнопка "➕ Add to channel" — модалка поиска канала и добавления | Готово | 2 (fix размера input в модалке) |
| 46 | Карточка пользователя открывается в правильном направлении без мигания: вверх если anchor в нижней половине экрана, иначе вниз; max-height + scroll для большого контента | Готово | 3 (useLayoutEffect мигал в Tauri; fix: синхронное вычисление направления по позиции anchor) |
| 43 | Custom Attributes плагин отдаёт данные как строки с эмодзи (:name:), карточка рендерит их через CustomEmojiRenderer; фиксированная ширина 280px, перенос строк | Готово | 1 |
| 44 | Ссылки в карточке пользователя (и везде в приложении) открываются в системном браузере через глобальный перехватчик кликов + Rust open_url команду | Готово | 1 |
| 47 | Quick Switcher (Cmd+P): пользователи отображаются в результатах поиска наряду с каналами | Готово | 1 (неправильные query-параметры в /users/autocomplete: team_id/term вместо in_team/name) |
| 48 | Кастомные эмодзи загружаются полностью и корректно рендерятся (поддержка `+` в именах, case-insensitive lookup, пагинация до пустой страницы) | Готово | 1 |
| 49 | Боковая панель информации о канале (кнопка i в шапке): Favorites/Mute/Copy Link, Purpose и Header с Markdown, channel_id, Notification Preferences, Members, Pinned Messages, Files | Готово | 1 |
| 50 | Исправлено отображение количества реплаев у корневых сообщений: reply_count теперь не перезаписывается меньшим значением при загрузке постов, используется Math.max при мерже | Готово | 1 |
| 51 | Кнопка "Move to Category" в контекстном меню канала: submenu с категориями теперь остаётся видимым при наведении; категории корректно отображаются (исправлен #[serde(rename)] конфликт: category_type vs type) | Готово | 2 (submenu исчезал из-за mouseLeave; категории не показывались из-за несоответствия имён полей Rust/TS) |
| 52 | Пустые категории отображаются в боковой панели (раньше скрывались если channel_ids пуст) | Готово | 1 |
| 53 | Уменьшено расстояние между категориями в боковой панели: margin-bottom убран, padding заголовка категории уменьшен до уровня высоты элемента канала | Готово | 2 (первая попытка не затронула padding заголовка) |
| 54 | Счётчик непрочитанных тредов отображается корректно: не завышается от нечитаемых тредов, правильно уменьшается при открытии, и показывается при запуске приложения | Готово | 2 (отображение исправлено в первой попытке; счётчик при запуске — во второй: store читался до loadTeams, activeTeamId был null) |
| 55 | Cmd+T открывает активный канал в новой вкладке (всегда создаёт новую); Cmd+W закрывает активную вкладку, кроме вкладки "по умолчанию" | Готово | 2 (Cmd+T не работал: openTab не создавал дубликат; fix: разделены id и channelId у вкладок, добавлен isDefault, новый метод openNewTab) |
| 56 | Вкладка с приватом (DM) отображает имя собеседника, а не своё имя | Готово | 1 (TabBar не передавал currentUserId — брал первого найденного юзера включая себя) |
| 57 | Исправлены описания шорткатов; пункт "Keyboard Shortcuts" добавлен в системное меню Window (Cmd+/) | Готово | 2 (Menu::os_default не существует в Tauri 2.10; fix: Menu::default; также Cmd+P/Cmd+K были перепутаны местами) |
| 58 | Кнопка прикрепить файл (📎) в композере треда; поддержка drag-and-drop и вставки изображений из буфера | Готово | 1 |
| 59 | «Open in Tab» в контекстном меню канала всегда создаёт новую вкладку, даже если канал уже открыт (в т.ч. является активным) | Готово | 1 |
| 60 | При закрытии окна оно скрывается в трей (а не закрывается), иконка трея живёт всё время жизни приложения; клик на иконку в Dock открывает скрытое окно (RunEvent::Reopen) | Готово | 2 (первый фикс: сохранение TrayIcon через app.manage(); второй: Dock click — обработчик RunEvent::Reopen через .build().run(callback)) |
| 61 | Кнопка Follow/Following в хедере ThreadPanel и рядом с "N replies" на корневом сообщении; синхронизация через WS событие thread_follow_changed (state + thread_id) | Готово | 3 (дефолт ?? true → ?? false; updateThreadFollowing upsert; WS событие называется thread_follow_changed, не thread_updated) |
| 64 | Перетаскивание канала из боковой панели в TabBar создаёт новый таб: кастомный drag через mousedown/mousemove/mouseup (HTML5 DnD и dragover/dragend заблокированы Tauri); ghost-элемент с именем канала следует за курсором; TabBar появляется как drop-зона даже когда табов нет; подсветка через getBoundingClientRect | Готово | 4 (HTML5 DnD заблокирован Tauri; dragover/dragend на window не работают; mouseup не срабатывает во время HTML5 drag; решение — полностью кастомный drag без HTML5 API) |
| 63 | Drag-and-drop файлов работает через Tauri `onDragDropEvent` с зональным hit-testом: один глобальный listener (`useTauriDragDrop` hook), drop-зона — только поле ввода composer'а (чат или тред); подсветка и приём файлов ограничены элементом composer'а через `getBoundingClientRect`; тред и чат независимы — файл падает туда, над чем находится курсор | Готово | 6 (HTML5 drop не даёт путей в Tauri webview; hit-test по координатам ломался из-за DPR; два listener'а конфликтовали; `position: relative` отсутствовал на .thread-panel-composer) |
| 62 | При открытии канала скролл идёт на последнее сообщение; если есть непрочитанные, сверху появляется кнопка "N new messages ↑" — клик перематывает на первое непрочитанное. Системные сообщения и reply не учитываются. Снэпшот `last_viewed_at` замораживается в отдельной мапе как в webapp (views.channel.lastChannelViewTime), view_channel вызывается только после загрузки постов | Готово | 5 (гонка view_channel с get_posts; стейл msg_count в локальном сторе; get_posts_around_last_unread возвращал не те посты; скролл до first unread перебивался ResizeObserver; финальное решение — скролл вниз + top-banner кнопка) |
| 65 | Редактирование сообщений в панели тредов: кнопка Edit на сообщениях, banner "Editing message" в composer, отмена по кнопке Cancel или Escape, вызов edit_post вместо send_post | Готово | 1 |
| 66 | В шапке ThreadPanel кнопка с именем канала (для DM — имя собеседника: nickname / First Last / username); клик переходит в канал; цвет кнопки совпадает с кнопкой Follow | Готово | 1 |
| 67 | В списке Threads View кнопка с именем канала после имени автора (для DM — ФИО/nickname собеседника, для каналов — # name); ширина 200px в ThreadsView, 120px в ThreadPanel | Готово | 1 |
| 68 | После отправки сообщения (или отмены редактирования) высота поля ввода сбрасывается до стандартной — в основном composer и в панели тредов | Готово | 1 |
| 69 | На корневых сообщениях с тредами отображаются аватарки участников (макс. 3); кликабельны — открывают карточку пользователя; загружаются фоном при открытии канала и при скролле вверх | Готово | 4 |
| 70 | При перетаскивании уже открытого канала в TabBar — активируется существующий таб вместо создания дубликата | Готово | 1 |
| 71 | Рендеринг Mattermost interactive messages (attachments) в чате и тредах: цветная полоса, автор, заголовок, текст, поля, изображения, футер, кнопки (button/select со стилями) — клик по кнопке вызывает POST /posts/{id}/actions/{actionId} | Готово | 2 (первая попытка не учитывала encodeURIComponent(actionId), metadata.embeds для определения типа, disabled у actions) |
| 72 | Поддержка slash-команд: автокомплит при вводе `/` в поле ввода (dropdown с trigger/hint/description), навигация стрелками, Tab/Enter для выбора, Escape для закрытия; отправка команды через POST /api/v4/commands/execute вместо /posts | Готово | 4 (неверный URL /commands/autocomplete_suggestions → /teams/{id}/commands/autocomplete_suggestions; структура ответа AutocompleteSuggestion с PascalCase полями вместо ожидаемой Command; поля десериализовались в snake_case но приходили как PascalCase из Tauri; regex не матчил `/cmd args` с пробелом) |
| 73 | Интерактивные диалоги (open_dialog) от slash-команд: парсинг WS-события open_dialog (dialog как JSON-строка), guard на elements в App.tsx; ephemeral_message через WS отображается в чате как обычный пост с кнопками; исправлены стили checkbox/radio (margin: 0, явный size 16px); overflow-x: hidden на модалке | Готово | 3 (trigger_id/url брались из вложенного dialog вместо data; WS-путь конфликтовал с polling; ephemeral_message был unhandled) |
| 74 | Клик по нотификации переводит в канал сообщения: нотификации через mac_notification_sys с wait_for_click (блокирует поток до клика); при клике — show/focus окно + emit notif:navigate-channel; каждая нотификация в отдельном потоке с привязкой к конкретному channelId | Готово | 8 (browser Notification onclick не работает в Tauri WebView; Tauri plugin onAction — desktop not supported; notify-rust — нет click handling на macOS; osascript — нет callback; Focused(true) — автопереключение без клика; итог: mac_notification_sys с wait_for_click + set_application("com.apple.Terminal")) |
| 75 | Исправлены баги нотификаций: (1) при двух нотификациях клик по первой открывал канал второй; (2) клик по старой нотификации после перезапуске крашил приложение; (3) нотификация не исчезала из центра уведомлений после клика. Решение: заменён mac_notification_sys на прямой objc2 (NSUserNotification + единый NSUserNotificationCenterDelegate), channel_id хранится в userInfo каждой нотификации, cold-start click буферизуется и доставляется после загрузки фронтенда | Готово | 2 (первая попытка через Tauri plugin sendNotification — desktop не поддерживает extra/onAction; вторая — objc2 define_class!) |
| 76 | Кастомные эмодзи: устойчивая постраничная загрузка списка (retry 3×, skip & continue), таймаут увеличен до 30с, виртуализированный грид в пикере (рендерятся только видимые ряды), LIFO-очередь загрузки изображений (видимые грузятся первыми), файловый кеш списка на диск (~/Library/Caches/) для мгновенной загрузки при перезапуске, фоновое обновление с сервера | Готово | 6 (React #185 из-за Map в zustand → Record; appendEmojis каскад → batch flush каждые 5 страниц; localStorage не persist в Tauri → файловый кеш через Rust; LIFO очередь с cancelled entries; identifier смена сломала нотификации → откат) |
| 77 | Исправлены дубликаты эмодзи в категориях пикера: sos ×3 в Symbols, cup_with_straw ×2 в Food, canoe ×2 в Travel — дублирование ключей React вызывало появление «призрачных» эмодзи при переключении категорий | Готово | 1 |
| 78 | Кнопка Reactions в боковой панели (под Threads): уведомления о реакциях других на мои сообщения; кастомные и стандартные эмодзи; аватарки пользователей; имя кликабельно (открывает DM), канал кликабельно (переходит в канал), аватарка кликабельна (карточка профиля); клик по уведомлению переходит на сообщение с подсветкой; загрузка исторических реакций при открытии через search API + get_reactions; бейдж непрочитанных в реальном времени через WS reaction_added | Готово | 1 |
| 79 | Реакции на посты обновляются в реальном времени: при WS-событии reaction_added/reaction_removed emoji-чипы под постом появляются/исчезают мгновенно (раньше обновлялась только панель Reactions, но не сами посты); обновление работает в messagesStore и threadsStore | Готово | 1 |
| 80 | Панель форматирования Markdown над полем ввода: кнопки Bold, Italic, Strikethrough, Heading, Link, Code, Quote, Bulleted list, Numbered list; работает в composer канала и в панели тредов; оборачивает выделенный текст или вставляет placeholder; Ctrl+B / Ctrl+I горячие клавиши; code — inline для одной строки, fenced block для многострочного | Готово | 1 |
| 81 | Рендеринг Mermaid-диаграмм в сообщениях: блоки ` ```mermaid ` отображаются как SVG-диаграммы (gantt, flowchart, sequence, pie и др.); тёмная тема; tab-переключатель Diagram/Code для просмотра исходного кода; при ошибке парсинга автоматически показывается исходный код с текстом ошибки | Готово | 4 (SVG терялся при переключении tab — fix: useState; regex извлекал mermaid из сырого текста и захватывал текстовые упоминания ` ```mermaid ` — fix: перехват в renderer.code по lang === "mermaid") |
| 82 | Автодополнение упоминаний (@): при вводе @ в поле ввода появляется список участников канала; поиск по логину и display name (включая кириллицу); аватарки пользователей; пометка BOT для ботов; разделение на секции "Channel Members" и "Not in Channel"; навигация стрелками с автопрокруткой, Tab/Enter для выбора, Escape для закрытия | Готово | 2 (первая попытка — базовый список без аватарок и секций; вторая — аватарки + BOT-бейдж + разделение + автопрокрутка при навигации) |
| 83 | При старте приложения исчезла ошибка "Не удалось загрузить сообщения — нет соединения с сервером": добавлен автоматический retry через 1.5с при transient-ошибках (connection reset / EOF) при первой загрузке сообщений канала (reqwest connection pool "холодный" в момент первого запроса) | Готово | 1 |
| 84 | Хоткей Quick Switcher изменён с Cmd+P на Cmd+K; обновлены: обработчик клавиш в ChannelList, tooltip и ярлык на кнопке Find, раздел Keyboard Shortcuts в ShortcutsModal | Готово | 1 |
| 85 | Slash-команды выполняются корректно через POST /commands/execute: исправлена передача team_id (обязательный параметр API) в тело запроса; исправлено несоответствие параметров Tauri handler (добавлен team_id: Option<String>), без которого invoke завершался ошибкой и сообщение не отправлялось | Готово | 2 |
| 86 | Кнопка поиска в боковой панели: текст изменён с "Find" на "Find channel or user" | Готово | 1 |
| 87 | Find, Threads, Reactions зафиксированы вверху боковой панели и не прокручиваются вместе со списком каналов: header вынесен в отдельный div с flex-shrink:0, список каналов обёрнут в scrollable div | Готово | 1 |
| 88 | Лимит отображаемых DM: select 10/15/20/40 в заголовке категории Direct Messages; по умолчанию 20; сохраняется в localStorage; работает в обоих режимах (category view и fallback) | Готово | 1 |
| 89 | DM сортируются по времени последнего сообщения (last_post_at desc) в обоих режимах отображения; сортировка применяется до обрезки по лимиту | Готово | 1 |
| 90 | Автодополнение каналов (~): при вводе ~ появляется dropdown с публичными и приватными каналами; поиск по name и display_name; навигация стрелками, Tab/Enter, Escape; вставляет ~channel-name в текст | Готово | 1 |
| 91 | @mention и ~channel в сообщениях отображают DisplayName вместо логина/имени канала | Готово | 1 |
| 92 | Клик по @DisplayName открывает карточку пользователя; клик по ~ChannelName переходит в канал | Готово | 1 |
| 93 | Карточка пользователя не смещается при подгрузке данных: позиция вычисляется один раз через useMemo при монтировании | Готово | 1 |
| 94 | Кнопка "Copy Link" в панели канала: после клика 2 сек показывает галочку, текст "Copied!" и зелёный цвет, затем возвращается в исходное состояние | Готово | 1 |
| 95 | Аватарки пользователей загружаются стабильно во всех местах одновременно: добавлена система подписок (listeners Map) в useUserAvatar — компоненты боковой панели получают аватарку сразу после её загрузки в любом другом месте приложения | Готово | 1 |
| 96 | Клик по реакции на сообщение в треде из панели Reactions открывает ThreadPanel с нужным тредом и подсвечивает конкретный ответ (root_id передаётся через Rust → store → навигацию) | Готово | 1 |
| 97 | Быстрый запуск (stale-while-revalidate): команда init_app_fast возвращает me + teams + кэш сессии с диска (~1 сек), UI показывается немедленно с данными прошлой сессии; load_channels_data запускает фоновый spawn, который параллельно загружает каналы + members + sidebar + favorites и по завершении эмитит Tauri-событие channels-loaded; фронт слушает событие и обновляет store; свежие данные сохраняются в session_{id}.json для следующего запуска | Готово | 1 |
| 98 | Скролл при открытии канала: shouldPinToBottom теперь сбрасывается только при ручном скролле вверх (а не по таймеру 2с), handleImageLoad проверяет shouldPinToBottom вместо жёсткого порога 300px — картинки больше не уводят скролл от последнего сообщения | Готово | 2 |
| 99 | Быстрое переключение каналов (stale-while-revalidate для постов): если канал уже посещался, старые посты показываются мгновенно без спиннера, новые подгружаются тихо в фоне | Готово | 1 |
| 100 | Исправлен выбор endpoint при открытии канала: вместо get_posts_around_last_unread (который при last_viewed_at==last_post_at возвращал только 30 старых постов и пропускал новые) теперь используется get_posts page=0 когда канал прочитан (last_post_at <= last_viewed_at); get_posts_around_last_unread используется только при реальных непрочитанных | Готово | 3 |
| 101 | Интерактивные/bot-сообщения (post_type != "" но не "system_*") отображались как пустой system-message вместо полноценного сообщения с attachments; исправлено: isSystem теперь true только для post_type начинающегося с "system_" | Готово | 1 |
| 102 | Системные сообщения "username joined/left the channel" скрыты: посты с типами system_join_channel, system_leave_channel, system_add_to_channel, system_remove_from_channel возвращают null в MessageItem | Готово | 1 |
| 103 | Мгновенное открытие каналов: (A) stale-while-revalidate — кэш из памяти или диска показывается сразу без спиннера, сеть обновляет тихо в фоне; (B) prefetch при hover — get_posts запускается при наведении мыши на канал; (C) параллельный fetch пользователей — get_users_by_ids идёт параллельно с загрузкой постов; (D) дисковый кэш постов — последние 60 постов каждого канала сохраняются на диск через load_posts_cache/save_posts_cache, мгновенно показываются после перезапуска | Готово | 3 (2 билда с ошибками компиляции) |
| 104 | Системные сообщения "joined/left the team" скрыты: типы system_join_team, system_leave_team, system_add_to_team, system_remove_from_team возвращают null в MessageItem | Готово | 1 |
| 105 | По нажатию ArrowUp в пустом поле ввода начинается редактирование последнего собственного сообщения — в канале (MessageComposer) и в тредах (ThreadPanel) | Готово | 3 (первый fix — пустые посты открывались; второй — открывалось первое сообщение вместо последнего: order newest-first, reverse() был лишним) |
| 106 | В панели треда вместо имени пользователя показывался короткий ID (8 символов): ThreadPanel не загружал данные авторов в кэш после получения постов; fix: после get_post_thread вызывается get_users_by_ids для авторов, отсутствующих в uiStore.users | Готово | 1 |
| 107 | В списке Members канала: клик по аватарке открывает карточку пользователя (UserPopover), кнопка ➤ при наведении открывает DM с участником; кнопка скрыта для себя | Готово | 1 |
| 108 | В списке Members канала администраторы отображаются первыми, затем остальные участники | Готово | 1 |
| 109 | Замьюченные каналы в боковой панели отображаются бледнее (opacity 0.45, курсив) и не показывают бейдж непрочитанных; Mute/Unmute доступен по кнопке в панели информации о канале и через контекстное меню; Mute = mark_unread:"mention" по Mattermost API; бэкенд передаёт notify_props.mark_unread из ChannelMember | Готово | 1 |
| 110 | Notification Preferences в панели канала: секция "Mute or ignore" (checkbox Mute channel + checkbox Ignore @channel/@here/@all), секция "Desktop Notifications" (radio All new messages / Mentions only); синхронизация через WS channel_member_updated; исправлен формат тела запроса PUT /channels/{id}/members/{user_id}/notify_props (плоский объект, не обёрнут в "notify_props"); mark_unread синхронизируется с локальным state через useEffect | Готово | 4 |
| 111 | Notification Preferences: если канал не замьючен — показываются секции "Notify me about..." (3 radio: all / mentions+DMs+keywords / nothing) и "Thread reply notifications" (checkbox); Thread reply показывается только при выборе "mentions only"; синхронизация всех полей notify_props через WS в реальном времени (store cache); исправлена двойная сериализация notify_props на стороне Rust (server возвращает JSON-строку); поле desktop_threads (не channel_auto_follow_threads) управляет checkbox тредов: "default"=включён, "mention"=выключён | Готово | 8 |
| 112 | Уведомления приходят только из незамьюченных каналов и при соответствии настройкам Notification (desktop: all/mention/none); системные сообщения (system_*) не генерируют уведомления; в заголовке уведомления из DM отображается "@username in Direct Message", из группового чата — "@username in [название группы]" | Готово | 1 |
| 113 | Кнопка "Filter by unread" (☰) в сайдбаре с хоткеем Cmd+U: показывает только незамьюченные каналы с непрочитанными сообщениями; кнопка слева от Find channel or user | Готово | 1 |
| 114 | Исправлена логика unread: unread (жирный текст) = total_msg_count > msg_count; mention (бейдж с числом) = только явные @username/@channel/@all/@here; системные сообщения (join/leave) не влияют на unread; используются total_msg_count_root / msg_count_root (Mattermost 6+) чтобы исключить системные сообщения из подсчёта при загрузке | Готово | 3 |
| 115 | Синхронизация unread между клиентами: обработка WS события multiple_channels_viewed (официальный клиент → наш клиент); view_channel API (наш → официальный); clearChannelUnread ставит msg_count = total_msg_count; get_channel_member для получения авторитетных счётчиков после внешнего viewed | Готово | 2 |
| 116 | Исправлен @ mention, ~channel, :emoji: autocomplete в тредах: ThreadPanel заменил встроенный упрощённый composer на shared MessageComposer с rootId prop; убран overflow: hidden с .thread-panel (клипал выпадающие списки, открывающиеся вверх) | Готово | 1 |
| 117 | Иконка кнопки "Add reaction" заменена с + на ☺; повторный клик закрывает пикер; пикер позиционируется через position:fixed с вычислением координат от viewport — открывается ниже кнопки если влезает, иначе выше; clamp по всем 4 краям экрана; тесты позиционирования (12 кейсов) в src/test/emojiPickerPosition.test.tsx | Готово | 6 |
| 118 | Защита от XSS: HTML после marked проходит через DOMPurify перед dangerouslySetInnerHTML (raw HTML в сообщениях больше не исполняется); легитимная разметка (упоминания, ссылки с target=_blank, code-блоки) сохраняется; 15 регрессионных тестов в src/test/markdownRenderer.test.tsx | Готово | 2 (дополнительное экранирование codespan дало двойное — marked уже экранирует на уровне токенизатора) |
| 119 | Строгий CSP в tauri.conf.json вместо csp:null: script-src 'self', картинки data:/blob: (аватарки приходят data-URL через Rust), unsafe-inline только для стилей (mermaid SVG), frame/object запрещены | Готово | 1 |
| 120 | Токены серверов хранятся в OS keychain (keyring: macOS Keychain / Windows Credential Manager / Linux Secret Service) вместо plaintext servers.json; существующие токены мигрируют автоматически при старте; фолбэк на конфиг если keychain недоступен; libdbus-1-dev добавлен в CI для Linux | Готово | 1 |
| 121 | @mention, ~channel и :emoji: не заменяются внутри код-блоков и inline-кода: препроцессинг применяется только к сегментам вне ``` ``` и `` ` `` (split по регулярке код-сегментов); +4 теста | Готово | 1 |
| 122 | ESLint (flat config: typescript-eslint + react-hooks) и Prettier установлены и настроены — скрипты lint/format раньше ссылались на отсутствующие пакеты; найдено и исправлено нарушение Rules of Hooks: два useMemo в ThreadPanel стояли после раннего return null; новые compiler-правила (set-state-in-effect, refs) понижены до warn (42 шт., техдолг) | Готово | 1 |
| 123 | CI ужесточён: убран `\|\| true` после фронтенд-тестов (падения не валили сборку), добавлен шаг ESLint; Rust-гейты доведены до зелёного: cargo fmt по всему бэкенду, 24 clippy-предупреждения закрыты (dead API-код помечен allow(dead_code), result_large_err разрешён на уровне crate) | Готово | 1 |
| 124 | Версия синхронизирована во всех манифестах (Cargo.toml отставал: 0.2.0 при 0.4.0 в package.json/tauri.conf.json); scripts/bump-version.js теперь бампит также Cargo.toml и Cargo.lock | Готово | 1 |
| 125 | Чистка репозитория: удалён случайно закоммиченный localhost-recording.json (490 КБ, WebKit timeline recording; паттерн добавлен в .gitignore), пустой LICENSE заполнен текстом MIT, удалён неиспользуемый react-router-dom, добавлен dompurify | Готово | 1 |
| 126 | В канале не отображаются разделители дат для дней, где нет ни одного видимого сообщения: раньше день, содержавший только входы/выходы (см. #102, #104) или только ответы в тредах, показывал пустой заголовок с датой без сообщений под ним. Правило видимости вынесено в `src/utils/postVisibility.ts` (`isRenderablePost` / `isHiddenSystemPost`) и применяется до вставки разделителя; MessageItem использует тот же предикат вместо своей копии проверки восьми типов. Побочно исправлено: скрытые посты больше не сбрасывают группировку сообщений (лишний аватар и имя после join между двумя сообщениями одного автора) и не расходятся со счётчиком «N new messages», который раньше отбрасывал все `system_*`, включая отображаемые | Готово | 1 |
| 127 | `handleDelete` и `handleImageLoad` добавлены в зависимости `useMemo` для `elements` в MessageList — оба стабильны (`useCallback` на `[serverId]` и `[]`), поэтому лишних пересчётов нет; предупреждений `react-hooks/exhaustive-deps` стало 5 вместо 6 | Готово | 1 |
| 128 | При клике по каналу с непрочитанным сообщением от бота окно чата иногда полностью пустое; прокрутка колесом вверх/вниз мгновенно отрисовывает содержимое | **Не воспроизведено** | 3 гипотезы, все опровергнуты тестами: (1) общий синглтон `EMPTY_ORDER` якобы отдавал закешированный пустой список из `useMemo` — неверно, `setChannelPosts` меняет ссылку `orderByChannel` и внешний `useMemo` пересчитывается; (2) схлопнутая высота контейнера в CSS — правила `.message-list` корректны; (3) гонка hover-prefetch (`prefetchChannelPosts` в ChannelList) с загрузкой канала, при которой `hasDataAlready` подавляет скролл вниз — опровергнуто: `applyPosts` перепиновывает список через ветку `hasDataAlready && !userScrolled.current`. Написанные под гипотезы тесты удалены (проходили и на исходном коде, то есть ничего не проверяли). Симптом «пусто до скролла» указывает на промах вьюпорта, а не на отсутствие DOM. Для диагностики при следующем воспроизведении нужны `document.querySelectorAll('.message-item').length` и `document.querySelector('.message-list').scrollTop` в момент бага — есть узлы в DOM или нет, отсекает половину пространства поиска |
| 129 | В разделе Threads отображаются настоящие аватарки авторов вместо кружка с первой буквой: `ThreadsView` рисовал `authorName.charAt(0)` в самодельном `.thread-list-avatar` и вообще не использовал компонент `UserAvatar`. Дополнительно `get_user_threads` не возвращает профили авторов, а `ThreadsView` их не догружал (та же нехватка, что и в #106 у ThreadPanel) — добавлен `fetchMissingAuthors` по образцу ThreadPanel. Третья часть: `users` читался разово через `useUiStore.getState().users`, поэтому список не перерисовывался после догрузки профилей — заменено на подписку `useUiStore((s) => s.users)`. 4 теста в `src/test/threadsViewAvatars.test.tsx`, все падают без фикса | Готово | 1 |
| 130 | Замьюченные личные диалоги (D) и групповые чаты (G) с непрочитанными сообщениями показываются в фильтре «Filter by unread» (⌘U), при этом остаются тихими в списке — без жирного шрифта и бейджа. Замьюченные обычные каналы по-прежнему в фильтр не попадают. Раньше `isUnread` безусловно отсекал всё замьюченное, и личка от замьюченного бота была недоступна через фильтр. Введён отдельный предикат `matchesUnreadFilter` (шире, чем `isUnread`, который продолжает управлять подсветкой); поведение регулируется настройкой Notifications → «Show muted DMs in unread filter», по умолчанию включена. Побочно: при активном фильтре не применяется лимит `dmLimit` (10/15/20/40) — иначе непрочитанная личка ниже лимита оставалась бы недоступной (в обоих режимах: категории и fallback). 9 тестов в `src/test/unreadFilterMutedDms.test.ts` | Готово | 1 |
| 131 | `saveSettings` в settingsStore не был обёрнут в try/catch (в отличие от `loadSettings`) — любое изменение настройки выбрасывало исключение там, где `localStorage` недоступен или заблокирован (приватный режим WebKit, запрет данных сайта). Обнаружено при написании тестов к #130 | Готово | 1 |
| 132 | Тумблер в настройках съезжал относительно соседних строк при длинном описании: у `.settings-toggle` не было `flex-shrink: 0`, поэтому фикс-ширина 40px сжималась, а `.settings-label` без `min-width: 0` растягивал колонку. Добавлены `flex-shrink: 0` у тумблера, `min-width: 0` у подписи и `gap: 16px` у строки — чинится для всех строк настроек, не только новой; текст описания заодно укорочен | Готово | 1 |
| 133 | Замьюченная личка с непрочитанными показывает бейдж с числом сообщений: бейдж был завязан на `mention_count`, а бот пишет без @-упоминаний, поэтому цифра не появлялась никогда. Введён `unreadBadgeCount` — для замьюченных D/G без упоминаний считает `total_msg_count − msg_count` (с защитой от отрицательных значений), в остальных случаях работает прежняя логика упоминаний. Бейдж горит всегда, не только под фильтром ⌘U; текст канала при этом остаётся бледным. 7 дополнительных тестов | Готово | 1 |
| 134 | Готовые сборки прикрепляются к GitHub Release: `tauri-action` вызывался без `tagName`, поэтому по тегу файлы уходили только в артефакты Actions, а у релиза не было ничего для скачивания. Передан `tagName` на `refs/tags/v*` — каждая matrix-задача грузит свой бандл в релиз; пуши в main собираются как раньше, релиза не трогают. Релиз при этом нужно создавать **до** окончания сборок, иначе четыре задачи создают его наперегонки | Готово | 1 |
| 135 | В `build.yml` добавлен `permissions: contents: write` — без него первая сборка по тегу v0.5.0 упала с `Resource not accessible by integration`: загрузка файлов в релиз это запись в contents, а дефолтный `GITHUB_TOKEN` только на чтение. Ошибка о причине ничего не сообщает | Готово | 1 |
| 136 | Восстановлена сборка под Linux и Windows: `RunEvent::Reopen` (клик по иконке в Dock, см. #60) существует только в macOS, поэтому компиляция падала с `E0599: no variant named Reopen`. Обёрнут в `#[cfg(target_os = "macos")]`. Баг лежал с момента добавления обработчика — релиз v0.5.0 стал первым, где собирались все четыре таргета, что его и вскрыло. Важно: локально (`./build.sh`, `cargo check`) проверяется только macOS, кросс-компиляции на машине нет — платформенный код подтверждается лишь через CI | Готово | 1 |
| 137 | После выхода ноутбука из сна (или любого разрыва соединения) непрочитанные каналы и личные сообщения не появляются: видно только то, что пришло уже после пробуждения, старое остаётся невидимым до ручного переключения канала. Причина: непрочитанные считались исключительно по WS-событиям `posted`, а Mattermost не переигрывает пропущенные события при переподключении — всё, что пришло, пока сокет лежал, существует только в счётчиках на сервере. Существовавший реконнект по `focus`/`online`/`visibilitychange` поднимал сокет, но состояние не перезапрашивал, а `ws_status` вообще никто не слушал. Добавлен `resyncUnreads()` — дёргает `load_channels_data` (каналы + members с авторитетными `msg_count`/`mention_count`, результат прилетает уже существующим событием `channels-loaded`) и `get_user_threads`; вызывается и по фокусу/сети, и по переходу `ws_status` в `connected` из любого другого состояния (бэкенд переподключается сам по idle-таймауту, где события фокуса нет). Побочно: `channels-loaded` теперь мержится через `mergeChannelsPreservingActive` — `view_channel` уходит асинхронно после загрузки постов, поэтому ресинк мог обогнать его и повесить бейдж на открытый канал, который пользователь читает. 5 тестов в `src/test/resyncChannels.test.ts` | Готово | 1 |
| 138 | При перезапуске приложения всегда открывался первый публичный канал в списке, а не тот, что был открыт перед закрытием. Выбор «первого канала с `channel_type === "O"`» был захардкожен в трёх местах: обработчик события `channels-loaded`, быстрый старт из кэша и `loadChannels`. Добавлена утилита `src/utils/lastChannel.ts` — `resolveInitialChannel` берёт запомненный канал, если он всё ещё есть в списке (мог быть покинут, заархивирован или переименован), иначе прежний фолбэк на первый публичный. Ключ хранения — пара сервер+команда, чтобы у разных серверов и команд был свой последний канал и id не протекал между ними; восстанавливаются и личные диалоги, не только публичные каналы. Запись повешена на `useEffect` по `activeChannelId` в сторе, а не на обработчик клика — так покрыты и переключение вкладок, и переход по уведомлению. Побочно: в `src/test/setup.ts` добавлен in-memory полифилл `localStorage` — jsdom здесь запускается без `--localstorage-file`, поэтому хранилище было `undefined` и любой тест персистентности падал (отсюда же `ExperimentalWarning` в каждом прогоне). Прототип `Storage.prototype` при этом не подменяется: у jsdom там нативные brand-проверки, и объект с таким прототипом бросает на любом вызове — тесты шпионят по самому инстансу. 10 тестов в `src/test/lastChannel.test.ts`, включая недоступное и битое хранилище | Готово | 1 |
| 139 | Видеофайлы проигрываются прямо в приложении вместо строки «📄 скачать»: новый компонент `src/components/message/VideoPlayer.tsx` с громкостью, скоростью (0.5×–2×), перемоткой, полноэкранным режимом, скачиванием и горячими клавишами (пробел/K — пауза, ←/→ — ±5 с, M — звук, F — полный экран). Ключевое решение — загрузка по клику: в ленте показывается постер с именем и размером, байты тянутся только когда пользователь нажал play, иначе канал с десятком роликов выкачивался бы весь сразу. Байты идут через новую Rust-команду `get_file_bytes` → `tauri::ipc::Response` (сырые байты, без base64: он раздувает объём на треть и гонит весь ролик через IPC строкой) → `Blob` → `URL.createObjectURL`, blob-URL кешируется по `fileId`, чтобы перемотка работала и плеер переживал ремаунты при прокрутке. Потребовалась правка CSP в `tauri.conf.json`: `blob:` был разрешён только для `img-src`, а для `<video>` действовал `default-src 'self'` — добавлен `media-src 'self' blob:`, без него видео молча не игралось бы. Формат определяется по расширению **и** по `mime_type`; список ограничен тем, что умеет WebKit (mp4/webm/ogv/m4v/mov) — avi/wmv/flv остаются обычными файлами, чтобы не показывать плеер, который упадёт на play. MIME берётся из file info, а не зашит как `video/mp4`. Автовоспроизведение только после явного клика (флаг `shouldAutoPlay`), иначе кешированный blob запускал бы ролик при прокрутке. 9 тестов в `src/test/videoPlayer.test.tsx` | Готово | 1 |
| 140 | Прочитанный тред продолжает считаться непрочитанным: пока панель треда открыта и собеседник пишет туда ответ, плашка «в тредах есть непрочитанное» не гаснет, а в левом меню под фильтром ⌘U канал этого же человека висит с непрочитанным. Две независимые причины. (1) `markThreadRead` вызывался только в `ThreadPanel` по смене `activeThreadId`, то есть один раз при открытии треда. Ответы, приходящие в уже открытую панель, шли через `addThreadReply` и ничего не сбрасывали — ни локальный `userThreadsUnread`, ни `last_viewed_at` на сервере. Добавлена `markOpenThreadRead` в `handlePosted`: при `activeThreadId === post.root_id` тред помечается прочитанным локально и командой `mark_thread_as_read`. Серверный вызов здесь важнее локального — без него следующий ресинк (`get_channel_member`, пробуждение, переподключение) возвращал канал обратно в непрочитанные. (2) В `handleChannelViewed`/`handleMultipleChannelsViewed` в `msg_count` писалось `member.msg_count` с сервера, тогда как `total_msg_count` бэкенд заполняет из `total_msg_count_root` (только корневые посты, Mattermost 6+). Сравнение `total_msg_count > msg_count` шло между разными шкалами, и канал выглядел непрочитанным ровно на число ответов в тредах. Добавлен `seenMsgCount` — берёт `msg_count_root`, откатываясь на `msg_count` на старых серверах, симметрично логике `ChannelWithMeta::from_channel_and_member` в Rust. Побочно: в `src/test/setup.ts` домокан `isPermissionGranted` — он вызывается на верхнем уровне `useWebSocket.ts`, без мока модуль не импортируется в тестах. 7 тестов в `src/test/threadUnreadOnOpenThread.test.ts` | Готово | 1 |

---

## Deferred / Not Yet Implemented

| Feature | Reason |
|---------|--------|
| Forward messages | Not yet implemented |
| Message reminders | Not yet implemented |
| Custom sidebar categories + drag-and-drop | Not yet implemented |
| Channel bookmarks | Not yet implemented |
| Deep linking (mattermost:// URL) | tauri-plugin-deep-link download failed (network) |
| Auto-update | tauri-plugin-updater download failed (network) |
| Multi-window pop-out | Deferred |
| GIF picker | Deferred |
| WebRTC Calls (Phase 11) | Not started |
| E2E test suite | Not started — обсуждалось 9 сентября 2026, отложено по решению пользователя (настоящий e2e через tauri-driver требует живого Mattermost с тестовыми данными) |
| Slash commands | Реализовано (автокомплит + выполнение) |
| Пустое окно чата до прокрутки (#128) | Открытый баг — не воспроизведён, причина не найдена; нужны данные из DevTools в момент проявления |

---

## Technical Workarounds

| Issue | Workaround |
|-------|-----------|
| `image` crate failed to download from crates.io | Used `png` crate directly (already cached) to decode PNG to raw RGBA for tray icon |
| `tauri-plugin-deep-link` download failed | Feature deferred |
| `tauri-plugin-updater` download failed | Feature deferred |
| Port 1420 conflict during dev | Identified and killed stale process |
| Context window exhausted 4 times | Sessions resumed via summary injection; some bugs had to be re-reported by user |
| 136 | Реакции и эмодзи с модификатором тона кожи отображались текстом (`:ok_hand_medium_light_skin_tone:`) вместо картинки: `EMOJI_MAP` содержит только базовые имена, а Mattermost присылает варианты как `<base>_<tone>_skin_tone`, которых в карте нет — `emojiNameToUnicode` не находил их и возвращал имя как есть. Варианты теперь выводятся из базового эмодзи на лету (модификаторы U+1F3FB–U+1F3FF), без раздувания карты на ~500 записей; фикс покрывает оба пути рендера — реакции (`ReactionsBar`) и текст сообщений (`MarkdownRenderer`), так как оба идут через одну функцию. Учтены два неочевидных случая: модификатор ставится **вместо** variation selector U+FE0F (`☝️` → `☝🏼`, иначе получается невалидная последовательность) и **перед** ZWJ-частью (`health_worker` 🧑‍⚕️ → 🧑🏼‍⚕️, а не 🧑‍⚕️🏼); суффиксы проверяются от длинного к короткому, иначе `_medium_light_skin_tone` разбирался как `_light_skin_tone` у базы `thumbsup_medium` (эта ошибка была в первой версии фикса и найдена тестом). Multi-person эмодзи (`family`) исключены — им нужен отдельный тон на каждую фигуру. 9 тестов в `src/test/emojiSkinTones.test.ts`, включая проверку всех 100 базовых эмодзи карты | Готово | 1 |
| 137 | CI на main падал на `lastChannel.test.ts:82` («treats unreadable storage as nothing remembered»: `expected 'c1' to be null`), при этом локально все тесты проходили — баг существовал до правок эмодзи (#136) и воспроизводился только в CI. Две причины, обе исправлены. (1) Второй `describe` («with unavailable storage») не имел `beforeEach` с `localStorage.clear()` — он был только в первом блоке, поэтому `c1`, записанный предыдущим тестом, читался обратно как запомненный канал. (2) Полифилл `MemoryStorage` в `src/test/setup.ts` стоял под условием `if (typeof globalThis.localStorage === "undefined")`, из-за чего среды расходились: на Node 26 (локально) `localStorage` отсутствует и подключается наш класс, чьи методы лежат на прототипе и корректно перехватываются `vi.spyOn`; на Node 22 (CI) `localStorage` предоставляет jsdom, полифилл не активировался, а его методы `vi.spyOn` не перехватывает (они за внутренним proxy, не на прототипе) — мок «заблокированной записи» молча пропускал реальную запись, и значение утекало в следующий тест. Условие снято: одна реализация хранилища в любой версии Node, иначе такие расхождения остаются невидимыми локально. Утечка подтверждена прямым воспроизведением (без `clear()` значение читается обратно, с ним — `null`) | Готово | 1 |
